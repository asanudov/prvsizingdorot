...
# ══════════════════════════════════════════════
#  CSS  —  sidebar más ancha, inputs legibles
# ══════════════════════════════════════════════
st.markdown("""<style>
...
def z_bnd(sigma_t, p2):
    return (sigma_t*p2+9)/(sigma_t-1)
...
def _card(label, value, unit=""):
    ...
    return f"""
    <div style='flex:1;min-width:0;border:1.5px solid #E2E8F0;border-radius:12px;
      ...
      <div style='font-size:9px;color:#94A3B8;font-weight:700;text-transform:uppercase;
                  letter-spacing:0.5px;margin-bottom:5px;overflow:hidden;
                  text-overflow:ellipsis;white-space:nowrap;' title='{label}'>{label}</div>
...
def cards_row(title, items, accent="#1B2A4A"):
    ...
    <div style='margin:10px 0 4px 0;'>
      ...
      <div style='display:flex;gap:8px;'>{inner}</div>
    </div>"""

def banner_zona(z, sig):
    ico,lbl,kind = ZONA_MAP[z]
    sv = f"σ = {sig:.1f}" if sig else "σ = —"
    getattr(st, kind)(f"{ico} **{lbl}** — {sv}")

# ══════════════════════════════════════════════
#  FIGURAS (compartidas entre app y PDF)
# ══════════════════════════════════════════════
...
def generate_pdf(logo_bytes, d):
    ...
    buf=io.BytesIO()
    PAGE_W,_=A4; MARGIN=1.8*cm; FW=PAGE_W-2*MARGIN
    ...
    GRN=C.HexColor('#28A745');   WARN=C.HexColor('#FD7E14')
    ERR=C.HexColor('#DC3545')

    def sty(name,**kw): return PS(name,**kw)
    ...
    story.append(hdr)
    story.append(HRFlowable(width=FW,thickness=3,color=TEAL,spaceAfter=4))
    ...
    if d['din']:
        ...
        e+=[['Caudal de diseño (Q)',f"{d['qmax_ui']:.1f}",d['u_q']],
            ['Presión de entrada P1',f"{d['p1min_ui']:.1f}",d['u_p']]]
    e+=[['Presión de ajuste P2',f"{d['p2_ui']:.1f}",d['u_p']],
        ['Diámetro de tubería',   f"{d['dn_mm']} mm",f"({d['dn_nom']})"],
    ...
    v=[['Parámetro','Valor'],
       ['Modelo','DOROT S300  (Mod. 30 / 31)'],
       ['Tamaño',f"{d['dn_mm']} mm  ({d['dn_nom']})"],
       ['Tipo de cuerpo',d['cuerpo']],
       ['Kv (apertura 100%)',str(d['kv_v'])],
       ['Cv (apertura 100%)',str(d['cv_v'])],
    ...
    story.append(ctbl([['Parámetro','Valor','Unidad'],
        ['Caudal Q',             f"{d['qmax_m3h']:.1f}",'m³/h'],
        ['Presión entrada P1',   f"{d['p1a_mca']:.1f}", 'mca'],
        ['Presión ajuste P2',    f"{d['p2_mca']:.1f}",  'mca'],
        ['Diferencia ΔP',        f"{d['dpa_mca']:.1f}", 'mca'],
        ['Kv requerido',         f"{d['kv_a']:.1f}",    'm³/h/√bar'],
        ['Kn / Kv',              f"{d['kn_a']:.3f}",    '—'],
        ['% Apertura',           f"{d['op_a']:.1f}",    '%'],
        ['Velocidad válvula',    f"{d['vel_a']:.1f}",   'm/s'],
        ['Índice σ cavitación',  f"{d['sig_a']:.2f}" if d['sig_a'] else '—','—'],
        ['Zona cavitación',      ZONA_MAP[d['z_a']][1], '']]))

    if d.get('has_b'):
        story.append(Spacer(1,0.15*cm))
        story.append(Paragraph("Escenario B — Qmín / P1 máx  (verificación cavitación)",H3))
        story.append(ctbl([['Parámetro','Valor','Unidad'],
            ['Caudal Q',             f"{d['qmin_m3h']:.1f}", 'm³/h'],
            ['Presión entrada P1',   f"{d['p1b_mca']:.1f}",  'mca'],
            ['Presión ajuste P2',    f"{d['p2_mca']:.1f}",   'mca'],
            ['Diferencia ΔP',        f"{d['dpb_mca']:.1f}",  'mca'],
            ['Kv requerido',         f"{d['kv_b']:.1f}",     'm³/h/√bar'],
            ['Kn / Kv',              f"{d['kn_b']:.3f}",     '—'],
            ['% Apertura',           f"{d['op_b']:.1f}",     '%'],
            ['Velocidad válvula',    f"{d['vel_b']:.1f}",    'm/s'],
            ['Índice σ cavitación',  f"{d['sig_b']:.2f}" if d.get('sig_b') else '—','—'],
            ['Zona cavitación',      ZONA_MAP[d['z_b']][1],  '']]))
    ...
    fc=make_cav_fig(d['p1a_mca'],d['p2_mca'],d['sig_a'],
                    d.get('p1b_mca'),d.get('sig_b'),d.get('has_b',False),figsize=(4.5,3.5))
    ...
    if za=='safe' and (not hb or zb=='safe'):
        concs.append(("✅ Cavitación — ","La válvula opera en condiciones seguras en todos los escenarios evaluados.",GRN))
    elif za=='destruct' or zb=='destruct':
        concs.append(("🚨 Cavitación — ","CAVITACIÓN DESTRUCTIVA. Implementar reducción en cascada (PRV en serie) o tapón U-Plug/V-Plug.",ERR))
    else:
        concs.append(("⚠️ Cavitación — ",f"Operación ruidosa (σ={d['sig_a']:.1f}). Considerar tapón V-Plug.",WARN))

    if 0.10<=d['kn_a']<=0.80:
        concs.append(("✅ Apertura A — ",f"Apertura {d['op_a']:.1f}% dentro del rango óptimo de regulación (10%–80%).",GRN))
    elif d['kn_a']>0.80:
        concs.append(("⚠️ Apertura A — ",f"Apertura {d['op_a']:.1f}% supera el límite. Evaluar tamaño mayor de válvula.",WARN))
    else:
        concs.append(("⚠️ Apertura A — ",f"Apertura {d['op_a']:.1f}% bajo mínimo. Evaluar tamaño menor de válvula.",WARN))

    if hb:
        if 0.10<=d['kn_b']<=0.80:
            concs.append(("✅ Apertura B — ",f"Apertura {d['op_b']:.1f}% dentro del rango óptimo.",GRN))
        else:
            concs.append(("⚠️ Apertura B — ",f"Apertura {d['op_b']:.1f}% fuera de rango. Verificar tamaño.",WARN))

    concs.append(("📐 Selección — ",
                  f"Válvula DOROT S300 de {d['dn_mm']} mm ({d['dn_nom']}). "
                  f"Kv requerido {d['kv_a']:.1f} vs Kv nominal {d['kv_v']} m³/h/√bar "
                  f"(Kn/Kv = {d['kn_a']:.3f}).",NAVY))
...
# ── Cálculos Escenario A
p1a_mca=fr_bar(p1a_bar,"mca"); p2_mca=fr_bar(p2_bar,"mca")
kv_a=calc_kv(qmax_m3h,dpa_bar); kn_a=kv_a/kv_v; op_a=calc_ap(kn_a)
sig_a=calc_sig(p1a_mca,p2_mca); z_a=zona(sig_a); vel_a=calc_vel(qmax_m3h,DN[idx])

# ── Cálculos Escenario B
...
if has_b:
    ...
    if dpb_bar<=0:
        ...
        sig_b=calc_sig(p1b_mca,p2_mca); z_b=zona(sig_b); vel_b=calc_vel(qmin_m3h,DN[idx])
...
st.markdown(cards_row("Datos de entrada",[
    ("Válvula",   f"{DN[idx]} mm", NOM[idx]),
    ("Kv (100%)", str(kv_v),       ""),
    ("Cv (100%)", str(cv_v),       ""),
]+vel_items), unsafe_allow_html=True)
...
if has_b:
    st.markdown(cards_row(
        "🅑 Parámetros — Qmín / P1 máx",
        [("ΔP",      f"{fr_bar(dpb_bar,u_p):.1f}", u_p),
         ("Kv req.", f"{kv_b:.1f}",                  ""),
         ("Kn/Kv",  f"{kn_b:.1f}",                  ""),
         ("Apertura",f"{op_b:.0f}",                  "%"),
         ("σ",       f"{sig_b:.1f}" if sig_b else "—","")],
        accent="#E67E22"
    ), unsafe_allow_html=True)
...
# ══════════════════════════════════════════════
#  DIAGNÓSTICO + COMPARATIVA
# ══════════════════════════════════════════════
st.divider(); st.subheader("🏥 Diagnóstico y recomendaciones")
...
with tcol:
    st.markdown("**Comparativa de tamaños**")
    ...
    for i in range(len(DN)):
        ...
        if has_b:
            ri_b=kv_b/kvi; oi_b=calc_ap(ri_b); ok=ok_a and 0.10<=ri_b<=0.80
            rows.append({"Tamaño":f"{DN[i]}mm ({NOM[i]})","Kv":kvi,
                         "Kn/Kv🅐":round(ri_a,2),"Ap🅐%":round(oi_a,0),
                         "Kn/Kv🅑":round(ri_b,2),"Ap🅑%":round(oi_b,0),
                         "":("✅ " if ok else "")+("← elegido" if i==idx else "")})
        else:
            ok=ok_a
            rows.append({"Tamaño":f"{DN[i]}mm ({NOM[i]})","Kv":kvi,
                         "Kn/Kv":round(ri_a,2),"Apert.%":round(oi_a,0),
                         "":("✅ " if ok else "")+("← elegido" if i==idx else "")})
...
with st.expander("📋 Ver tabla de resultados completa"):
    ...
    filas=[("Válvula",f"{DN[idx]} mm — {NOM[idx]}"),("Cuerpo",cuerpo),
           ("Kv (100%)",str(kv_v)),("Cv (100%)",str(cv_v)),
           ...
           ("ΔP",f"{fr_bar(dpa_bar,u_p):.1f} {u_p}"),
           ("Kv req.",f"{kv_a:.1f}"),("Kn/Kv",f"{kn_a:.3f}"),
    ...
    if has_b:
        filas+=[("— ESC. B —",""),
                ...
                ("ΔP",f"{fr_bar(dpb_bar,u_p):.1f} {u_p}"),
                ("Kv req.",f"{kv_b:.1f}"),("Kn/Kv",f"{kn_b:.3f}"),
...
st.caption("Incluye datos de entrada · válvula seleccionada · parámetros de cálculo · gráficas · conclusiones")
...
with c1:
    if st.button("📄 Generar PDF", type="primary", use_container_width=True):
        rd = {
            "din":din,"u_q":u_q,"u_p":u_p,"cuerpo":cuerpo,
            ...
            "p2_ui":p2_ui,"dn_mm":DN[idx],"dn_nom":NOM[idx],
            "kv_v":kv_v,"cv_v":cv_v,
        ...
        if not st.session_state.pdf_bytes:
            st.error("Error al generar PDF. Verifica que `reportlab` esté instalado.")
with c2:
    if st.session_state.pdf_bytes:
        st.download_button(
            "⬇️ Descargar PDF",
            data=st.session_state.pdf_bytes,
            file_name=f"PRV_DOROT_S300_{DN[idx]}mm_{date.today().strftime('%Y%m%d')}.pdf",
